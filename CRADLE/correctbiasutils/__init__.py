...
def getScalerTasks(trainingSets, ctrlBWNames, experiBWNames, sampleCount):
	...
	for i in range(1, sampleCount):
		tasks.append([i, trainingSets, ob1Values, ctrlBWNames, experiBWNames])

	return tasks

def getScalerForEachSample(taskNum, trainingSets, ob1Values, ctrlBWNames, experiBWNames):
	if taskNum < len(ctrlBWNames):
		ob2 = pyBigWig.open(ctrlBWNames[taskNum])
	else:
		ob2 = pyBigWig.open(experiBWNames[taskNum - len(ctrlBWNames)])
...
def getCandidateTrainingSet(rcPercentile, binSize, regions, ctrlBWNames, outputDir):
	trainBinSize = max(TRAINING_BIN_SIZE, int(binSize) * 2)
	...
	# trainRegionNum = int(np.round(trainRegionNum * 0.5 / 5))
	trainingRegionNum = int(np.round(trainRegionNum * 0.5 / 9))
	trainingSetMeta = []

	ctrlBW = pyBigWig.open(ctrlBWNames[0])
	...
	rc90Percentile = None
	rc99Percentile = None
	for i in range(len(rcPercentile) - 1):
		rc1 = int(np.percentile(meanRC, int(rcPercentile[i])))
		rc2 = int(np.percentile(meanRC, int(rcPercentile[i+1])))

		if rcPercentile[i] == 90:
			rc90Percentile = rc1

		if rcPercentile[i] == 99:
			temp = [rc1, rc2, 3*trainingRegionNum, regions, ctrlBWNames[0], outputDir, rc90Percentile, rc99Percentile]
			rc99Percentile = rc1
		else:
			temp = [rc1, rc2, trainingRegionNum, regions, ctrlBWNames[0], outputDir, rc90Percentile, rc99Percentile] # RC criteria1(down), RC criteria2(up), # of bases, candidate regions
